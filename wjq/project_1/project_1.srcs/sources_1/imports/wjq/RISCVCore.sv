...
module InsMEM(    //指令存储器
    ...
    // {rom[3],rom[2],rom[1],rom[0]}     = 32'b0000000 00100 00000 000 00001 0010011;   //ADDI x1, x0; 将零寄存器(x0)的值 +4 存入x1 
    // {rom[7],rom[6],rom[5],rom[4]}     = 32'b0000000 00000 00001 000 00010 0110011;   //ADD x3, x0, x1; 将x0与x1相加结果存入x3 
    // {rom[11],rom[10],rom[9],rom[8]}   = 32'b0000000 00010 00001 010 00010 0100011;   //SW x10, 4(x1); 将x10的值存入内存地址 x1 +4 =8
    // {rom[15],rom[14],rom[13],rom[12]} = 32'b0000000 00010 00001 010 00011 0000011;   //LW x1, 4(x1); 从内存地址 x1 +4 =8 加载数据到x1 → x1 = MEM[8]
    // {rom[19],rom[18],rom[17],rom[16]} = 32'b0000000 00010 00011 000 00100 0110011;   //ADD x5, x2, x17 ; 将x2与x17相加结果存入x5
    // {rom[23],rom[22],rom[21],rom[20]} = 32'b0000000 01000 00000 000 00101 0010011;   //ADDI x2, x0, 8; 将零寄存器(x0)的值 +8 存入x2
    // {rom[27],rom[26],rom[25],rom[24]} = 32'b0100000 00001 00101 000 00110 0110011;   //SUB x6, x1, x4; 将x1与x4相减结果存入x6
    // {rom[31],rom[30],rom[29],rom[28]} = 32'b0000000 00000 00000 000 00000 1100011;   //BEQ x0, x0, 12; 若x0与x0相等则跳转到PC+12
